...
export default {
  ...
  async submit(ctx: any) {
    ...
    if (task.status_task !== "In Progress") return ctx.badRequest("งานนี้ไม่อยู่ในสถานะที่ส่งได้");
    ...
    if (!files?.proofImage) return ctx.badRequest("กรุณาแนบรูปภาพหลักฐาน");
  ...
  async approve(ctx: any) {
    ...
    if (!task) return ctx.notFound("ไม่พบงานนี้");
    if (task.status_task !== "Under Review") return ctx.badRequest("งานนี้ไม่ได้รอการตรวจอยู่");

    const updated = await updateTask(id, { status_task: "Done" }, ["current_owner", "project"]);
    return ctx.send({ data: updated });
  },
  ...
  async handover(ctx: any) {
    ...
    if (!task) return ctx.notFound("ไม่พบงานนี้");
    if (task.status_task !== "In Progress") return ctx.badRequest("ส่งไม้ต่อได้เฉพาะงานที่กำลังดำเนินอยู่");
    if (task.current_owner?.id !== user.id) return ctx.forbidden("คุณไม่ใช่เจ้าของงานนี้");
  ...
  async requestPickup(ctx: any) {
    ...
    if (!task) return ctx.notFound("ไม่พบงานนี้");
    if (task.status_task !== "Waiting for Pickup") return ctx.badRequest("งานนี้ไม่ได้รอคนรับช่วงต่อ");
    if (task.handover_requested_by?.id) return ctx.badRequest("มีคนขอรับงานนี้แล้ว กรุณารอหัวหน้าอนุมัติ");
    ...
    if (!membership) return ctx.forbidden("คุณไม่ได้เป็นสมาชิกโปรเจกต์นี้");
  ...
  async cancelPickup(ctx: any) {
    ...
    if (!isRequester && !isManager && !isServiceRole) return ctx.forbidden("ไม่มีสิทธิ์ยกเลิกคำขอนี้");
  ...
  async approvePickup(ctx: any) {
    ...
    const task = await findTask(id, ["handover_requested_by", "project"]);
    if (!task) return ctx.notFound("ไม่พบงานนี้");
    if (task.status_task !== "Waiting for Pickup") return ctx.badRequest("งานนี้ไม่ได้รอรับช่วงต่อ");
    if (!task.handover_requested_by?.id) return ctx.badRequest("ยังไม่มีคนขอรับงานนี้");

    const updated = await updateTask(id, {
      status_task: "In Progress",
      current_owner: task.handover_requested_by.id,
      handover_requested_by: null,
      handover_at: null,
  ...
  async signedUrl(ctx: any) {
    ...
    const task = await findTask(id);
    if (!task) return ctx.notFound("ไม่พบงานนี้");
    if (!task.task_image_url) return ctx.badRequest("งานนี้ยังไม่มีรูปหลักฐาน");
